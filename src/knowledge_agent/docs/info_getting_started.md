...
<a id="databases"></a>
## 1. Connect the databases

Knowledge Agent keeps your knowledge base in two places:

- **Neo4j** (the knowledge graph) — a separate database you point the app at.
- **LanceDB** (the vector store) — embedded, saved to a folder on disk, no server
  to run.

Open **Settings → Database connection** and set the Neo4j address, username and
password, plus the LanceDB path. See [connecting Neo4j and LanceDB](#connect)
below for detail.
...
base. Then select it in the **Selected corpus** dropdown (top-right) so every tab
works on it.
...
embeds each document into LanceDB and extracts entities and relationships into
Neo4j. Larger corpora take longer — you can watch progress and cancel a folder
run.
...
3. In Knowledge Agent, open **Settings → Database connection** and enter that bolt
   URL, the username (`neo4j` by default), and the password you set.
...
**LanceDB (the vector store)** needs no download and no server — it's an
*embedded* store saved to a folder. Just set the **LanceDB path** in the same
Settings section; the folder is created on your first ingest. That's the split:
...
or connect to beyond the path. In the default setup neither database is a cloud
service — your documents and graph stay on your machine.
