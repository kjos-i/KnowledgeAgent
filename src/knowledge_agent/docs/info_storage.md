...
| `lancedb/` | the vector store (chunks + embeddings), rebuilt from your documents on ingest (see [row schema below](#schema)) |
...
automatically, so you don't have to memorise it.
...
| `(:OntologyTerm)-[:<X>_IS_A]->(:OntologyTerm)` | ontology hierarchy, variable-depth with `*` (most ontologies use `<X>_IS_A`; MeSH uses `MESH_BROADER`) |
