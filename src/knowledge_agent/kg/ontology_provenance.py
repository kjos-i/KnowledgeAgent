...
`kg/ontology_*_writes.py` module builds a `_<NAME>_PROVENANCE =
OntologyProvenance(...)` constant at module load time, and any import
back through `ontology_lifecycle.py` (which imports the registry which
imports those very modules) would cycle.

`ontology_lifecycle.py` re-exports `OntologyProvenance` from this
...
class OntologyProvenance:
    ...
    Fields:
      - `ontology_name`: registry key (e.g. "mesh"). Matches the
        `ONTOLOGY_REGISTRY` dict key in `ontology_linking.py`.
