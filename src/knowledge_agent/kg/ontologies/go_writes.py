...
`helpers.py` (refactored 2026-06-23 alongside HPO/UBERON/
MONDO/ChEBI). The only per-ontology surface is the constants block
below + the thin function wrappers.

Re-exports `ensure_cached` from `helpers` so existing test
...
GO_ID_PREFIX = "GO"
# Surfaced by `lifecycle.import_ontology_plan` ("GO (~200 MB
