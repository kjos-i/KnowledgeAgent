...
domain - alongside the `*_writes.py` adapters and the registry in
`linking.py`.
...
`linking.py`); adding a new ontology means one new entry in
...
from knowledge_agent.kg.ontologies.helpers import ensure_cached, get_downloads_dir
...
from knowledge_agent.kg.ontologies.xrefs import (
...
# The dataclass itself lives in the leaf `provenance` module so
...
async def install_xrefs_plan(client, new_xrefs_mode: str) -> InstallXrefsPlan:
    ...
    `kg.ontologies.xrefs` diagnostics. Per-ontology fail-soft (a None
...
# (declared per-ontology — see kg/ontologies/provenance.py) joined
