...
Three-file split (refactor 2026-06-29):
  - `helpers.py` (THIS FILE) — leaf shared by all three
    families: `OntologyTerm` dataclass + download/cache utilities.
    Slimmed from its historical role (~930 lines) to ~140 lines.
  - `pronto.py` — pronto-based OBO reading + extraction.
  - `rdf.py` — rdflib-based RDF/OWL/SKOS reading + extraction.
...
# against `knowledge_agent.kg.ontologies.helpers`.
#
...
# Pronto OBO family.
from knowledge_agent.kg.ontologies.pronto import (  # noqa: E402
    extract_terms_obo,
    read_obo,
)

# rdflib RDF / OWL / SKOS family.
from knowledge_agent.kg.ontologies.rdf import (  # noqa: E402
...
# Neo4j writes family.
from knowledge_agent.kg.ontologies.writes import (  # noqa: E402
...
__all__ = [
    ...
    "_validate_xrefs_mode",
    "_xref_rel_from_term_label",
    # Writes family (re-exported from writes)
    "delete_ontology_terms",
    "ensure_cached",
    # Pronto family (re-exported from pronto)
    "extract_terms_obo",
    # rdflib family (re-exported from rdf)
    "extract_terms_owl",
    "extract_terms_skos",
    "get_downloads_dir",
    "import_ontology_data",
