...
INFO: dict[str, InfoText] = {
    ...
    "ingest.bulk_chunks": InfoText(
        ...
        standard=(
            "These two buttons work on the corpus's existing chunks (the pieces each "
            "document was split into) — without re-reading the original files.\n\n"
            "Re-embed all chunks re-computes the search vectors for every chunk with the "
            "current "
            "embedding model and refreshes the search index. Use it after switching the "
            "embedding model or its settings; it doesn't change the chunks or the "
            "knowledge graph. (It can't change the vector size — that needs a full "
            "re-ingest, and it reports those docs as failures.)\n\n"
            "Rebuild graph from chunks rebuilds the knowledge-graph side from the "
            "existing chunks: it re-creates the chunk nodes, then re-runs everything "
            "built from them — entities, ontology links, triples, and both "
            "cross-document layers — so the graph matches your current settings "
            "(rebuilding the layers that are on, clearing any you've turned off). Use it "
            "if the graph was wiped or you turned a graph layer on after adding "
            "documents. It doesn't re-split or re-read the files — to change chunk size "
            "or the splitter, do a full Re-ingest."
        ),
        beginner=(
            "Every document you add is split into small pieces called chunks, and each "
            "chunk gets a numeric 'fingerprint' (an embedding) used for searching. These "
            "two buttons refresh those from the chunks already stored — no need to "
            "re-add the files.\n\n"
            "Re-embed all chunks makes fresh fingerprints for every chunk with the "
            "current settings "
            "— use it if you changed which embedding model the app uses, so search keeps "
            "working well. It only updates the search side, not the text or the "
            "knowledge graph.\n\n"
            "Rebuild graph from chunks rebuilds the knowledge-graph parts made from your "
            "chunks — the entities found in them, links to reference vocabularies, "
            "relationships, and connections between documents — so the graph reflects "
            "your current settings. Use it if the graph was cleared, or you switched a "
            "graph feature on (or off) after adding documents, and you want it brought "
            "up to date from the chunks you already have. It doesn't re-split your "
            "documents (for that, re-add them)."
        ),
        technical=(
            "Both read chunk data from LanceDB; neither re-parses the source.\n\n"
            "Re-embed all chunks (bulk_re_embed → re_embed): re-embeds each doc's chunks "
            "via the "
            "configured provider and delete-then-inserts to swap the LanceDB vector "
            "column (also refreshes ingested_at, re-optimizes the index if "
            "optimize_indexes_per_ingest). Refuses a dimension change before the delete "
            "(lancedb_ok=False → counted a failure; change dims via full re-ingest); "
            "rolls back on a failed rewrite. No KG cascade — nothing in the graph reads "
            "the vector.\n\n"
            "Rebuild graph from chunks (bulk_backfill_chunks → backfill_chunks): "
            "reconciles the downstream layers to config first (wipes L6-L10 data for any "
            "layer now off), then per doc deletes + rewrites the KG :Chunk/:PART_OF from "
            "existing LanceDB chunks and cascades backfill_entities (L6) → "
            "backfill_ontology (L7) + backfill_triples (L8) + backfill_cross_doc (L9) + "
            "backfill_cross_doc_xrefs (L10), each a no-op when its layer is off. Does "
            "NOT re-chunk (chunk_max_tokens / chunker_strategy changes need re-ingest)."
        ),
    ),
    "ingest.bulk_entities": InfoText(
        ...
        standard=(
            ...
            "happen: first, any downstream layer you've switched off (ontology, triples, "
            "cross-document links) is cleared across the whole corpus. Then, document by "
            "document, it deletes that document's old data at each stage before writing "
            "the fresh version — its old entities/mentions, then its old triples, then "
            "its old cross-document links — so old and new are never mixed. Each layer "
            "is only rebuilt if it's turned on.\n\n"
            "Use it after you change the entity extractor or the entity types, or after "
            "enabling entities (or a layer below it) once documents are already in. It "
            "reads the stored chunks, so it can't change how documents are split — that "
            "needs a full Re-ingest.\n\n"
            "(Entities are shared between documents, so this refreshes each document's "
            "own mentions but keeps entities other documents still use.)"
        ),
        beginner=(
            ...
            "the chunks already stored (no need to re-add the files).\n\n"
        ...
        technical=(
            ...
            "data), like ingest. reconcile_entities also prunes entity types no longer "
            "allowed.\n\n"
            "Then per doc, each stage deletes this doc's old data before rewriting: "
            "backfill_entities runs delete_entities_by_doc_id (drops this doc's "
            ":MENTIONS, GCs entities left with no mentions from any chunk — :Entity nodes "
            "are corpus-shared/MERGEd, so ones other docs mention survive), then re-runs "
            "extract_union (corpus's extractors / entity_types / entity_types_mode / "
            "model + temperature) and writes fresh :Entity + :MENTIONS. The cascade "
            "repeats the pattern, each a no-op when its layer is off: backfill_triples "
            "(delete_triples_by_doc_id → re-extract), backfill_cross_doc / "
            "backfill_cross_doc_xrefs (wipe this doc's :RELATED_TO / :RELATED_BY_XREF → "
            "recompute), backfill_ontology (re-link via MERGE, idempotent). Empty chunks "
            "skip the extractor; per-chunk extraction is fail-soft. Reads chunks from "
            "LanceDB — no re-parse; chunk_max_tokens / chunker_strategy changes need a "
            "full Re-ingest."
        ),
    ),
