...
async def test_import_hpo_writes_term_nodes_via_pronto_obo_path(
    ...
    _skip_unless_downloaded(hpo_writes.HPO_CACHE_FILENAME, "HPO")

    # Clean: drop any prior HPO state so the test exercises the
    # write path (not the already-imported short-circuit).
    await hpo_writes.delete_imported(kg_client)

    ok = await hpo_writes.import_hpo(kg_client, force=True)
    assert ok is True

    # is_imported now returns True.
    assert await hpo_writes.is_imported(kg_client) is True
...
async def test_hpo_import_idempotent_via_ensure_imported(
    ...
    _skip_unless_downloaded(hpo_writes.HPO_CACHE_FILENAME, "HPO")
    await hpo_writes.delete_imported(kg_client)
    await hpo_writes.import_hpo(kg_client, force=True)
    ...
    await hpo_writes.import_hpo(kg_client, force=False)
...
async def test_hpo_delete_imported_drops_every_term_node(
    ...
    await hpo_writes.delete_imported(kg_client)
    assert await hpo_writes.is_imported(kg_client) is False
...
async def test_import_efo_writes_term_nodes_via_rdflib_owl_path(
    ...
    terms imported."""
    _skip_unless_downloaded(efo_writes.EFO_CACHE_FILENAME, "EFO")
    await efo_writes.delete_imported(kg_client)
    ok = await efo_writes.import_efo(kg_client, force=True)
    assert ok is True

    assert await efo_writes.is_imported(kg_client) is True
...
async def test_import_mesh_writes_term_nodes_via_streaming_n_triples_path(
    ...
    _skip_unless_downloaded(mesh_writes.MESH_CACHE_FILENAME, "MeSH")
    await mesh_writes.delete_imported(kg_client)
    ok = await mesh_writes.import_mesh(kg_client, force=True)
    assert ok is True

    assert await mesh_writes.is_imported(kg_client) is True
...
async def test_import_fibo_writes_term_nodes_via_multi_file_walker_path(
    ...
    _skip_unless_downloaded(fibo_writes.FIBO_CACHE_SUBDIR, "FIBO")
    await fibo_writes.delete_imported(kg_client)
    ok = await fibo_writes.import_fibo(kg_client, force=True)
    assert ok is True

    assert await fibo_writes.is_imported(kg_client) is True
