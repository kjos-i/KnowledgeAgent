...
async def _seed_doc(client: Any, doc_id: str, entities: list[tuple[str, str]]) -> str:
...
async def test_recompute_writes_related_to_edge_when_threshold_met(
    ...
    await _seed_doc(kg_client, DOC_ALPHA, SHARED_ENTITIES + ALPHA_UNIQUE)
    await _seed_doc(kg_client, DOC_BETA, SHARED_ENTITIES + BETA_UNIQUE)
...
async def test_recompute_handles_multi_doc_neighbourhood(
    ...
    incident to alpha."""
    await _seed_doc(kg_client, DOC_ALPHA, SHARED_ENTITIES)
    await _seed_doc(kg_client, DOC_BETA, SHARED_ENTITIES)
    await _seed_doc(kg_client, DOC_GAMMA, SHARED_ENTITIES)
    ...
    async with kg_client.driver.session() as session:
        ...
        neighbours = [r async for r in result]
