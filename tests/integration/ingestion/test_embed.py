...
async def test_embed_texts_returns_correct_dimension() -> None:
    ...
    vectors = await embed_texts(["one short input"])
    assert vectors is not None
    assert len(vectors) == 1
    assert len(vectors[0]) == expected_dim


async def test_embed_texts_empty_input_returns_empty_list_no_api_call() -> None:
    """Empty input is the fast-path optimization — no Voyage call."""
    vectors = await embed_texts([])
    assert vectors == []


async def test_embed_texts_document_vs_query_input_type_both_succeed() -> None:
    ...
    doc_vec = await embed_texts([text], input_type="document")
    query_vec = await embed_texts([text], input_type="query")
    assert doc_vec is not None
    assert query_vec is not None
    assert len(doc_vec[0]) == len(query_vec[0])


async def test_embed_texts_vectors_are_floats_in_finite_range() -> None:
