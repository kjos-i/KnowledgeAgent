...
def test_ingest_folder_plan_lists_one_file(
    ...
    plan = ingest_folder_plan(fresh_corpus_dir, "Document", "Paper")
...
def test_ingest_folder_execute_runs_pipeline_per_file(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    result = ingest_folder_execute(plan, config)
...
def test_sync_plan_classifies_unchanged_file(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    plan = ingest_folder_plan(fresh_corpus_dir, "Document", "Paper")
    config = _minimal_corpus_config()
    ingest_folder_execute(plan, config)

    # Now sync — file hasn't changed.
    splan = sync_plan(fresh_corpus_dir, "Document", "Paper")
...
def test_sync_plan_classifies_moved_file(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    sresult = sync_execute(splan, config)
    assert sresult.n_moved == 1


def test_sync_plan_classifies_orphan_when_file_removed(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
...
def test_delete_doc_plan_reports_inventory(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
...
def test_delete_doc_execute_wipes_both_stores(
    ...
    doc_id = compute_doc_id(fresh_corpus_dir / sample_pdf.name)
    dplan = delete_doc_plan(doc_id)
    result = delete_doc_execute(dplan)
...
def test_bulk_resolve_openalex_plan_targets_indexed_docs(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    iplan = ingest_folder_plan(fresh_corpus_dir, "Document", "Paper")
    ingest_folder_execute(iplan, _minimal_corpus_config())
...
def test_bulk_resolve_openalex_execute_against_real_openalex(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
...
def test_bulk_re_embed_plan_includes_chunk_count(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
...
def test_bulk_re_embed_execute_preserves_chunk_count(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    ingest_folder_execute(iplan, _minimal_corpus_config())
    doc_id = compute_doc_id(fresh_corpus_dir / sample_pdf.name)
    pre_count = len(lance_client.get_chunks_by_doc_id(doc_id) or [])
    ...
    post_count = len(lance_client.get_chunks_by_doc_id(doc_id) or [])
...
def test_bulk_backfill_chunks_succeeds_after_chunks_layer_enabled(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    ingest_folder_execute(iplan, config)

    plan = bulk_backfill_chunks_plan()
    assert isinstance(plan, BulkBackfillPlan)
    assert plan.n_targets == 1
    result = bulk_backfill_chunks_execute(plan, config)
    assert result.n_succeeded == 1
    assert result.n_failed == 0
...
def test_bulk_backfill_entities_creates_entity_nodes(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
...
def test_bulk_backfill_ontology_links_entities_to_pre_seeded_mesh_terms(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    ingest_folder_execute(iplan, entities_cfg)
    ...
    result = bulk_backfill_ontology_execute(plan, mesh_cfg)
...
def test_bulk_backfill_triples_succeeds_after_l6a_present(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    result = bulk_backfill_triples_execute(plan, triples_cfg)
...
def test_bulk_backfill_cross_doc_runs_recompute_per_doc(
    fresh_corpus_dir: Path,
    kg_client: Any,
    lance_client: Any,
    clean_both_stores: None,
    ...
    result = bulk_backfill_cross_doc_execute(plan, cross_cfg)
    assert result.n_succeeded == 1
    assert result.n_failed == 0
...
def test_backfill_xrefs_resolves_dangling_xrefs(
    ...
    plan = backfill_xrefs_plan(config)
    assert isinstance(plan, BackfillXrefsPlan)
    assert plan.n_dangling_sources >= 1

    result = backfill_xrefs_execute(plan, config)
...
def test_backfill_xrefs_skips_when_xrefs_mode_none(
    kg_client: Any, ensure_constraints: None
) -> None:
...
def test_recompute_cross_doc_xrefs_when_layer_disabled_is_noop(
    kg_client: Any, ensure_constraints: None
) -> None:
    ...
    plan = recompute_cross_doc_xrefs_plan(config)
...
def test_recompute_cross_doc_xrefs_writes_edges_with_synthetic_setup(
    kg_client: Any, ensure_constraints: None
) -> None:
...
def test_clear_xref_edges_drops_target_ontology_edges(
    kg_client: Any, ensure_constraints: None
) -> None:
    ...
    plan = clear_xref_edges_plan("mondo")
