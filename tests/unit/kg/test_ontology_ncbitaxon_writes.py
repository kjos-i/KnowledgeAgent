"""Tests for kg.ontology_ncbitaxon_writes - the L7 NCBITaxon adapter."""
...
from knowledge_agent.kg import ontology_ncbitaxon_writes
...
def test_domain_tags_declared():
    assert isinstance(ontology_ncbitaxon_writes.DOMAIN_TAGS, tuple)
    assert "biology" in ontology_ncbitaxon_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await ontology_ncbitaxon_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await ontology_ncbitaxon_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await ontology_ncbitaxon_writes.is_imported(_client_with_driver(driver))


async def test_is_imported_query_uses_term_label():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    client = _client_with_driver(driver)
    await ontology_ncbitaxon_writes.is_imported(client)
    cypher, _ = driver.sessions[0].calls[0]
    assert ":NCBITaxonTerm" in cypher


async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await ontology_ncbitaxon_writes.write_terms(_client_with_driver(driver), []) is None
    assert driver.sessions == []


async def test_write_terms_one_round_trip_when_no_hierarchy():
    driver = RecordingDriver()
    terms = [_term("NCBITaxon:1", "root")]
    assert await ontology_ncbitaxon_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_two_round_trips_when_hierarchy_present():
    driver = RecordingDriver()
    terms = [
        _term("NCBITaxon:9605", "Homo"),
        _term("NCBITaxon:9606", "Homo sapiens", parents=("NCBITaxon:9605",)),
    ]
    assert await ontology_ncbitaxon_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    driver = RecordingDriver()
    terms = [_term("NCBITaxon:9606", "Homo sapiens", synonyms=("human",))]
    await ontology_ncbitaxon_writes.write_terms(_client_with_driver(driver), terms)
    cypher, params = driver.sessions[0].calls[0]
    assert ":OntologyTerm" in cypher
    assert ":NCBITaxonTerm" in cypher
    assert params["rows"] == [{"id": "NCBITaxon:9606", "label": "Homo sapiens",
                               "synonyms": ["human"], "definition": None}]
...
async def test_write_terms_edge_query_uses_is_a_rel():
    ...
    assert ":NCBITAXON_IS_A" in cypher
    assert params["rows"] == [{"child": "NCBITaxon:9606", "parent": "NCBITaxon:9605"}]


async def test_write_terms_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await ontology_ncbitaxon_writes.write_terms(
            _client_with_driver(driver), [_term("NCBITaxon:1", "root")])
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert await ontology_ncbitaxon_writes.import_ncbitaxon(_client_with_driver(driver), force=False) is False
...
async def test_import_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontology_writes.ensure_cached", return_value="/fake/ncbitaxon.obo"),
        patch("knowledge_agent.kg.ontology_ncbitaxon_writes._read_and_extract",
              return_value=[_term("NCBITaxon:1", "root")]),
    ):
        assert await ontology_ncbitaxon_writes.import_ncbitaxon(_client_with_driver(driver), force=True) is True
    assert len(driver.sessions) == 2
    delete_cypher, _ = driver.sessions[0].calls[0]
    assert ":NCBITaxonTerm" in delete_cypher
...
async def test_import_aborts_on_zero_terms():
    ...
    with (
        patch("knowledge_agent.kg.ontology_writes.ensure_cached", return_value="/fake/ncbitaxon.obo"),
        patch("knowledge_agent.kg.ontology_ncbitaxon_writes._read_and_extract", return_value=[]),
    ):
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            await ontology_ncbitaxon_writes.import_ncbitaxon(_client_with_driver(driver), force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await ontology_ncbitaxon_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":NCBITaxonTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await ontology_ncbitaxon_writes.delete_imported(_client_with_driver(driver))


async def test_client_is_imported_delegates_to_module():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    assert await _client_with_driver(driver).is_ncbitaxon_imported() is True


async def test_client_delete_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await client.delete_ncbitaxon() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":NCBITaxonTerm" in cypher
    assert "DETACH DELETE" in cypher
...
async def test_client_import_delegates_to_module():
    ...
    client = _client_with_driver(driver)
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert await client.import_ncbitaxon(force=False) is False
    mc.assert_not_called()
