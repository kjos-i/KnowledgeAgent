...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    assert ":NCBITaxonTerm" in cypher
    assert params["rows"] == [
        {"id": "NCBITaxon:9606", "label": "Homo sapiens", "synonyms": ["human"], "definition": None}
    ]
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontology_writes.require_cached") as mc:
        assert (
            await ontology_ncbitaxon_writes.import_ncbitaxon(
                _client_with_driver(driver), force=False
            )
            is False
        )
...
async def test_import_force_drops_then_reimports():
    ...
    with (
        patch(
            "knowledge_agent.kg.ontology_writes.require_cached", return_value="/fake/ncbitaxon.obo"
        ),
        patch(
            "knowledge_agent.kg.ontology_ncbitaxon_writes._read_and_extract",
            return_value=[_term("NCBITaxon:1", "root")],
        ),
    ):
        assert (
            await ontology_ncbitaxon_writes.import_ncbitaxon(
                _client_with_driver(driver), force=True
            )
            is True
        )
