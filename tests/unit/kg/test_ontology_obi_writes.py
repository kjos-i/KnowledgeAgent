"""Tests for kg.ontology_obi_writes - the L7 OBI adapter (OWL/rdflib path).
...
def test_domain_tags_declared():
    assert isinstance(ontology_obi_writes.DOMAIN_TAGS, tuple)
    assert "experimental-methods" in ontology_obi_writes.DOMAIN_TAGS
...
def test_is_imported_true_when_query_returns_present():
    ...
    assert ontology_obi_writes.is_imported(_client_with_driver(driver)) is True
...
def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert ontology_obi_writes.is_imported(_client_with_driver(driver)) is False
...
def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        ontology_obi_writes.is_imported(_client_with_driver(driver))


def test_is_imported_query_uses_obiterm_label():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    client = _client_with_driver(driver)
    ontology_obi_writes.is_imported(client)
...
def test_write_terms_empty_input_returns_true_no_io():
    ...
    assert ontology_obi_writes.write_terms(_client_with_driver(driver), []) is None
...
def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert ontology_obi_writes.write_terms(_client_with_driver(driver), terms) is None
...
def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    ontology_obi_writes.write_terms(_client_with_driver(driver), terms)
...
def test_write_terms_edge_query_uses_obi_is_a_rel():
    driver = RecordingDriver()
    terms = [
        _term("OBI:0000123", "process A"),
        _term("OBI:0000456", "process B", parents=("OBI:0000123",)),
...
def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        ontology_obi_writes.write_terms(
            _client_with_driver(driver), [_term("OBI:0000123", "process A")])


def test_import_obi_short_circuits_when_already_imported():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert ontology_obi_writes.import_obi(_client_with_driver(driver), force=False) is False
    mc.assert_not_called()


def test_import_obi_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        ...
        assert ontology_obi_writes.import_obi(_client_with_driver(driver), force=True) is True
...
def test_import_obi_aborts_on_zero_terms():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": False}])]])
    with (
        patch("knowledge_agent.kg.ontology_writes.ensure_cached", return_value="/fake/obi.owl"),
        ...
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            ontology_obi_writes.import_obi(_client_with_driver(driver), force=False)


def test_import_obi_propagates_download_exception():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": False}])]])
...
def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert ontology_obi_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":OBITerm" in cypher
...
def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        ontology_obi_writes.delete_imported(_client_with_driver(driver))
