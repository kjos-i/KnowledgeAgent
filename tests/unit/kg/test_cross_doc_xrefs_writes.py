...
async def test_per_doc_empty_doc_id_raises():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    with pytest.raises(ValueError, match="no doc_id"):
        await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(client, "")
    assert driver.sessions == []


async def test_per_doc_threshold_zero_raises():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
...
async def test_per_doc_threshold_negative_raises():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    with pytest.raises(ValueError, match=">= 1"):
        await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(
...
async def test_per_doc_propagates_cypher_exception():
...
async def test_per_doc_returns_zero_when_single_returns_no_row():
...
async def test_per_doc_runs_two_queries_delete_then_merge():
...
async def test_per_doc_zero_edges_is_valid_outcome():
...
async def test_per_doc_delete_query_uses_related_by_xref_and_undirected():
    driver = _driver_with_per_doc_count(0)
    client = _client_with_driver(driver)
    await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(client, DOC_ID)
...
async def test_per_doc_merge_query_carries_label_disjunction_on_both_sides():
...
async def test_per_doc_merge_query_walks_canonical_to_ontology_term_path():
...
async def test_per_doc_merge_query_uses_xref_equivalence_clause():
...
async def test_per_doc_merge_query_filters_self_edges():
    driver = _driver_with_per_doc_count(0)
    client = _client_with_driver(driver)
    await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(client, DOC_ID)
...
async def test_per_doc_merge_query_uses_threshold_param():
...
async def test_per_doc_merge_query_sets_shared_concepts_count_and_timestamp():
...
async def test_per_doc_merge_query_uses_undirected_merge():
    driver = _driver_with_per_doc_count(0)
    client = _client_with_driver(driver)
    await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(client, DOC_ID)
    merge_cypher, _ = driver.sessions[0].calls[1]
...
async def test_per_doc_uses_default_threshold_when_not_provided():
    driver = _driver_with_per_doc_count(0)
    client = _client_with_driver(driver)
    await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_edges(client, DOC_ID)
...
async def test_global_threshold_zero_raises():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    with pytest.raises(ValueError, match=">= 1"):
...
async def test_global_runs_two_queries_delete_then_merge():
    """Global wipes ALL :RELATED_BY_XREF edges before recompute."""
    driver = RecordingDriver(single_returns=[None, {"n": 17}])
    client = _client_with_driver(driver)
    result = await cross_doc_xrefs_writes.recompute_cross_doc_xrefs_global(client)
    assert result == 17
    assert len(driver.sessions[0].calls) == 2
...
async def test_global_merge_uses_id_pair_dedup():
...
async def test_global_merge_uses_equivalence_clause():
...
async def test_global_propagates_cypher_exception():
    """Driver failure propagates; orchestrator boundary catches."""
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
...
async def test_global_returns_zero_when_single_returns_no_row():
    """Defensive fallback when the driver returns no row — returns 0
    (not None)."""
    driver = RecordingDriver(single_returns=[None, None])
    client = _client_with_driver(driver)
