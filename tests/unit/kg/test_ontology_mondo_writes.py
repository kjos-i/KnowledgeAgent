...
def test_is_imported_true_when_query_returns_present():
    ...
    assert ontology_mondo_writes.is_imported(client) is True
...
def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert ontology_mondo_writes.is_imported(client) is False
...
def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        ontology_mondo_writes.is_imported(client)


def test_is_imported_query_uses_mondoterm_label():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
...
def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert ontology_mondo_writes.write_terms(client, []) is None
...
def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert ontology_mondo_writes.write_terms(client, terms) is None
...
def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    ontology_mondo_writes.write_terms(client, terms)
...
def test_write_terms_edge_query_uses_mondo_is_a_rel():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    terms = [
        _term("MONDO:0000001", "disease or disorder"),
    ...
    assert params["rows"] == [
        {"child": "MONDO:0005148", "parent": "MONDO:0000001"}
    ]
...
def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        ontology_mondo_writes.write_terms(
...
def test_import_mondo_short_circuits_when_already_imported():
    ...
    with patch(
        ...
        result = ontology_mondo_writes.import_mondo(client, force=False)

    assert result is False  # no-op: typed-errors contract
    mock_cache.assert_not_called()


def test_import_mondo_force_drops_then_reimports():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    ...
    with (
        ...
        result = ontology_mondo_writes.import_mondo(client, force=True)
...
def test_import_mondo_aborts_on_zero_terms():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": False}])]
        ]
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            ontology_mondo_writes.import_mondo(client, force=False)


def test_import_mondo_propagates_download_exception():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": False}])]
        ]
...
def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert ontology_mondo_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":MONDOTerm" in cypher
...
def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        ontology_mondo_writes.delete_imported(client)


# ---- Neo4jClient delegate methods ----


def test_client_is_mondo_imported_delegates_to_module():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
    ...
    client = _client_with_driver(driver)
    assert client.is_mondo_imported() is True


def test_client_delete_mondo_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert client.delete_mondo() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":MONDOTerm" in cypher
    assert "DETACH DELETE" in cypher


def test_client_import_mondo_delegates_to_module():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
    ...
    with patch(
        ...
        assert client.import_mondo(force=False) is False
    mock_cache.assert_not_called()
