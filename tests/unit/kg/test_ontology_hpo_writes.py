...
def test_is_imported_true_when_query_returns_present():
    ...
    assert ontology_hpo_writes.is_imported(client) is True
...
def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert ontology_hpo_writes.is_imported(client) is False
...
def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        ontology_hpo_writes.is_imported(client)


def test_is_imported_query_uses_hpoterm_label():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
...
def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert ontology_hpo_writes.write_terms(client, []) is None
...
def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert ontology_hpo_writes.write_terms(client, terms) is None
...
def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    ontology_hpo_writes.write_terms(client, terms)
...
def test_write_terms_edge_query_uses_hpo_is_a_rel():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    terms = [
        _term("HP:0000118", "Phenotypic abnormality"),
...
def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        ontology_hpo_writes.write_terms(
...
def test_import_hpo_short_circuits_when_already_imported():
    ...
    with patch(
        ...
        result = ontology_hpo_writes.import_hpo(client, force=False)

    assert result is False  # no-op: typed-errors contract
    mock_cache.assert_not_called()


def test_import_hpo_force_drops_then_reimports():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    ...
    with (
        ...
        result = ontology_hpo_writes.import_hpo(client, force=True)
...
def test_import_hpo_aborts_on_zero_terms():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": False}])]
        ]
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            ontology_hpo_writes.import_hpo(client, force=False)


def test_import_hpo_propagates_download_exception():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": False}])]
        ]
...
def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert ontology_hpo_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":HPOTerm" in cypher
...
def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        ontology_hpo_writes.delete_imported(client)
