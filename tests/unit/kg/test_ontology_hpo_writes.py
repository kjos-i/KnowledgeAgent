"""Tests for kg.ontology_hpo_writes - the L7 HPO adapter.
...
from knowledge_agent.kg import ontology_hpo_writes
...
def test_domain_tags_declared():
    ...
    assert isinstance(ontology_hpo_writes.DOMAIN_TAGS, tuple)
    assert len(ontology_hpo_writes.DOMAIN_TAGS) > 0
    assert "medicine" in ontology_hpo_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await ontology_hpo_writes.is_imported(client) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert await ontology_hpo_writes.is_imported(client) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        await ontology_hpo_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await ontology_hpo_writes.write_terms(client, []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await ontology_hpo_writes.write_terms(client, terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await ontology_hpo_writes.write_terms(client, terms)
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await ontology_hpo_writes.write_terms(
            client, [_term("HP:0000118", "Phenotypic abnormality")]
        )
...
async def test_import_hpo_force_drops_then_reimports():
    ...
    with (
        ...
        patch(
            "knowledge_agent.kg.ontology_hpo_writes._read_and_extract",
            return_value=fake_terms,
        ),
    ):
        result = await ontology_hpo_writes.import_hpo(client, force=True)
...
async def test_import_hpo_aborts_on_zero_terms():
    ...
    with (
        ...
        await ontology_hpo_writes.import_hpo(client, force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await ontology_hpo_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":HPOTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        await ontology_hpo_writes.delete_imported(client)
