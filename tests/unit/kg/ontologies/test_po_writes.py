"""Tests for kg.ontologies.po_writes - the L7 PO adapter."""
...
from knowledge_agent.kg.ontologies import po_writes
...
def test_domain_tags_declared():
    assert isinstance(po_writes.DOMAIN_TAGS, tuple)
    assert "biology" in po_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await po_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await po_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await po_writes.is_imported(_client_with_driver(driver))
...
async def test_is_imported_query_uses_term_label():
    ...
    await po_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await po_writes.write_terms(_client_with_driver(driver), []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await po_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await po_writes.write_terms(_client_with_driver(driver), terms)
    cypher, params = driver.sessions[0].calls[0]
    assert ":OntologyTerm" in cypher
    assert ":POTerm" in cypher
    assert params["rows"] == [
        {"id": "PO:0009001", "label": "leaf", "synonyms": ["foliage leaf"], "definition": None}
    ]
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert await po_writes.import_po(_client_with_driver(driver), force=False) is False
...
async def test_import_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/po.obo"),
        patch(
            "knowledge_agent.kg.ontologies.po_writes._read_and_extract",
            return_value=[_term("PO:0025131", "plant anatomical entity")],
        ),
    ):
        assert await po_writes.import_po(_client_with_driver(driver), force=True) is True
...
async def test_import_aborts_on_zero_terms():
    ...
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/po.obo"),
        patch("knowledge_agent.kg.ontologies.po_writes._read_and_extract", return_value=[]),
    ):
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            await po_writes.import_po(_client_with_driver(driver), force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await po_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":POTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await po_writes.delete_imported(_client_with_driver(driver))
