"""Tests for kg.ontologies.mondo_writes - the L7 MONDO adapter.
...
from knowledge_agent.kg.ontologies import mondo_writes
...
def test_domain_tags_declared():
    ...
    assert isinstance(mondo_writes.DOMAIN_TAGS, tuple)
    assert len(mondo_writes.DOMAIN_TAGS) > 0
    assert "medicine" in mondo_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await mondo_writes.is_imported(client) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert await mondo_writes.is_imported(client) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        await mondo_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await mondo_writes.write_terms(client, []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await mondo_writes.write_terms(client, terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await mondo_writes.write_terms(client, terms)
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await mondo_writes.write_terms(client, [_term("MONDO:0000001", "disease or disorder")])
...
async def test_import_mondo_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mock_cache:
        result = await mondo_writes.import_mondo(client, force=False)
...
async def test_import_mondo_force_drops_then_reimports():
    ...
    with (
        ...
        patch(
            "knowledge_agent.kg.ontologies.mondo_writes._read_and_extract",
            return_value=fake_terms,
        ),
    ):
        result = await mondo_writes.import_mondo(client, force=True)
...
async def test_import_mondo_aborts_on_zero_terms():
    ...
    with (
        ...
        await mondo_writes.import_mondo(client, force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await mondo_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":MONDOTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        await mondo_writes.delete_imported(client)
