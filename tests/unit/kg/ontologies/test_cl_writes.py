"""Tests for kg.ontologies.cl_writes - the L7 CL adapter."""
...
from knowledge_agent.kg.ontologies import cl_writes
...
def test_domain_tags_declared():
    assert isinstance(cl_writes.DOMAIN_TAGS, tuple)
    assert "cell biology" in cl_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await cl_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await cl_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await cl_writes.is_imported(_client_with_driver(driver))
...
async def test_is_imported_query_uses_term_label():
    ...
    await cl_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await cl_writes.write_terms(_client_with_driver(driver), []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await cl_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await cl_writes.write_terms(_client_with_driver(driver), terms)
    cypher, params = driver.sessions[0].calls[0]
    assert ":OntologyTerm" in cypher
    assert ":CLTerm" in cypher
    assert params["rows"] == [
        {"id": "CL:0000540", "label": "neuron", "synonyms": ["nerve cell"], "definition": None}
    ]
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await cl_writes.write_terms(_client_with_driver(driver), [_term("CL:0000000", "cell")])
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert await cl_writes.import_cl(_client_with_driver(driver), force=False) is False
...
async def test_import_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/cl.obo"),
        patch(
            "knowledge_agent.kg.ontologies.cl_writes._read_and_extract",
            return_value=[_term("CL:0000000", "cell")],
        ),
    ):
        assert await cl_writes.import_cl(_client_with_driver(driver), force=True) is True
...
async def test_import_aborts_on_zero_terms():
    ...
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/cl.obo"),
        patch("knowledge_agent.kg.ontologies.cl_writes._read_and_extract", return_value=[]),
    ):
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            await cl_writes.import_cl(_client_with_driver(driver), force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await cl_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":CLTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await cl_writes.delete_imported(_client_with_driver(driver))
...
async def test_client_is_imported_delegates_to_module():
    ...
    assert await _client_with_driver(driver).is_cl_imported() is True


async def test_client_delete_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await client.delete_cl() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":CLTerm" in cypher
    assert "DETACH DELETE" in cypher
...
async def test_client_import_delegates_to_module():
    ...
    client = _client_with_driver(driver)
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert await client.import_cl(force=False) is False
    mc.assert_not_called()
