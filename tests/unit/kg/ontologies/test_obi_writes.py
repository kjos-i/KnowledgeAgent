"""Tests for kg.ontologies.obi_writes - the L7 OBI adapter (OWL/rdflib path).
...
from knowledge_agent.kg.ontologies import obi_writes
...
def test_domain_tags_declared():
    assert isinstance(obi_writes.DOMAIN_TAGS, tuple)
    assert "experimental-methods" in obi_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await obi_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await obi_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await obi_writes.is_imported(_client_with_driver(driver))
...
async def test_is_imported_query_uses_obiterm_label():
    ...
    await obi_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await obi_writes.write_terms(_client_with_driver(driver), []) is None
    assert driver.sessions == []


async def test_write_terms_one_round_trip_when_no_hierarchy():
    driver = RecordingDriver()
    terms = [_term("OBI:0000123", "process A")]
    assert await obi_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_two_round_trips_when_hierarchy_present():
    driver = RecordingDriver()
    terms = [
        _term("OBI:0000123", "process A"),
        _term("OBI:0000456", "process B", parents=("OBI:0000123",)),
    ]
    assert await obi_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await obi_writes.write_terms(_client_with_driver(driver), terms)
...
async def test_import_obi_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert await obi_writes.import_obi(_client_with_driver(driver), force=False) is False
    mc.assert_not_called()


async def test_import_obi_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/obi.owl"),
        patch(
            "knowledge_agent.kg.ontologies.obi_writes._read_and_extract",
            return_value=[_term("OBI:0000123", "process A")],
        ),
    ):
        assert await obi_writes.import_obi(_client_with_driver(driver), force=True) is True
    assert len(driver.sessions) == 2
    delete_cypher, _ = driver.sessions[0].calls[0]
    assert ":OBITerm" in delete_cypher
...
async def test_import_obi_aborts_on_zero_terms():
    ...
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/obi.owl"),
        patch("knowledge_agent.kg.ontologies.obi_writes._read_and_extract", return_value=[]),
    ):
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            await obi_writes.import_obi(_client_with_driver(driver), force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await obi_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":OBITerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await obi_writes.delete_imported(_client_with_driver(driver))


def test_read_and_extract_uses_rdflib_not_pronto():
    """Critical invariant: OBI's `_read_and_extract` must go through
    rdflib (read_rdf + extract_terms_owl), NOT pronto's `read_obo` +
    `extract_terms_obo`. Pronto silently drops oboInOwl synonyms
    from OWL files; rdflib preserves them."""
    from pathlib import Path

    with (
        patch("knowledge_agent.kg.ontologies.obi_writes.read_rdf") as mock_read_rdf,
        patch(
            "knowledge_agent.kg.ontologies.obi_writes.extract_terms_owl", return_value=[]
        ) as mock_extract,
    ):
        obi_writes._read_and_extract(Path("/fake/obi.owl"))
    mock_read_rdf.assert_called_once()
    mock_extract.assert_called_once()
    # And confirm the format hint is RDF/XML.
