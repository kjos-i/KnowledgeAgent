"""Tests for kg.ontologies.helpers - the L7 import foundation.
...
def _patch_http_stream(*, chunks: list[bytes] | None = None, raise_mid: bool = False):
    """Patch `knowledge_agent.kg.ontologies.helpers._http_client.stream` with
    ...
    return patch(
        "knowledge_agent.kg.ontologies.helpers._http_client.stream",
...
def test_get_downloads_dir_creates_directory(tmp_path: Path):
    ...
    with patch("knowledge_agent.kg.ontologies.helpers.get_settings") as mock_settings:
...
async def test_ensure_cached_returns_existing_file(tmp_path: Path):
    ...
    with (
        patch("knowledge_agent.kg.ontologies.helpers.get_settings") as mock_settings,
        patch("knowledge_agent.kg.ontologies.helpers._http_client.stream") as mock_stream,
    ):
        mock_settings.return_value.ontology_downloads_dir = target
...
async def test_write_ontology_terms_batches_large_node_writes(monkeypatch):
    ...
    from knowledge_agent.kg.ontologies import writes

    monkeypatch.setattr(writes, "_WRITE_BATCH_SIZE", 2)
    client = _StubClient()
...
async def test_delete_ontology_terms_resets_canonicalised_on_orphaned_entities():
    ...
    client = _StubClient()
    await writes.delete_ontology_terms(client, term_label="XTerm", ontology_name="X")
