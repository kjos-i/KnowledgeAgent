"""Tests for kg.ontologies.envo_writes - the L7 ENVO adapter."""
...
from knowledge_agent.kg.ontologies import envo_writes
...
def test_domain_tags_declared():
    assert isinstance(envo_writes.DOMAIN_TAGS, tuple)
    assert "environment" in envo_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await envo_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await envo_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await envo_writes.is_imported(_client_with_driver(driver))
...
async def test_is_imported_query_uses_term_label():
    ...
    await envo_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await envo_writes.write_terms(_client_with_driver(driver), []) is None
    assert driver.sessions == []


async def test_write_terms_one_round_trip_when_no_hierarchy():
    driver = RecordingDriver()
    terms = [_term("ENVO:01000254", "environmental system")]
    assert await envo_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_two_round_trips_when_hierarchy_present():
    driver = RecordingDriver()
    terms = [
        _term("ENVO:01000254", "environmental system"),
        _term("ENVO:00000873", "freshwater biome", parents=("ENVO:01000254",)),
    ]
    assert await envo_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    driver = RecordingDriver()
    terms = [_term("ENVO:00000873", "freshwater biome", synonyms=("freshwater ecosystem",))]
    await envo_writes.write_terms(_client_with_driver(driver), terms)
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert await envo_writes.import_envo(_client_with_driver(driver), force=False) is False
...
async def test_import_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/envo.obo"),
        patch(
            "knowledge_agent.kg.ontologies.envo_writes._read_and_extract",
            return_value=[_term("ENVO:01000254", "environmental system")],
        ),
    ):
        assert await envo_writes.import_envo(_client_with_driver(driver), force=True) is True
...
async def test_import_aborts_on_zero_terms():
    ...
    with (
        patch("knowledge_agent.kg.ontologies.writes.require_cached", return_value="/fake/envo.obo"),
        patch("knowledge_agent.kg.ontologies.envo_writes._read_and_extract", return_value=[]),
    ):
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            await envo_writes.import_envo(_client_with_driver(driver), force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await envo_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":ENVOTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await envo_writes.delete_imported(_client_with_driver(driver))
...
async def test_client_delete_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await client.delete_envo() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":ENVOTerm" in cypher
    assert "DETACH DELETE" in cypher
