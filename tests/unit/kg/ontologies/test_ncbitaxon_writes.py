"""Tests for kg.ontologies.ncbitaxon_writes - the L7 NCBITaxon adapter."""
...
from knowledge_agent.kg.ontologies import ncbitaxon_writes
...
def test_domain_tags_declared():
    assert isinstance(ncbitaxon_writes.DOMAIN_TAGS, tuple)
    assert "biology" in ncbitaxon_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await ncbitaxon_writes.is_imported(_client_with_driver(driver)) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert await ncbitaxon_writes.is_imported(_client_with_driver(driver)) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        await ncbitaxon_writes.is_imported(_client_with_driver(driver))
...
async def test_is_imported_query_uses_term_label():
    ...
    await ncbitaxon_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    assert await ncbitaxon_writes.write_terms(_client_with_driver(driver), []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await ncbitaxon_writes.write_terms(_client_with_driver(driver), terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await ncbitaxon_writes.write_terms(_client_with_driver(driver), terms)
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await ncbitaxon_writes.write_terms(
...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mc:
        assert (
            await ncbitaxon_writes.import_ncbitaxon(_client_with_driver(driver), force=False)
            is False
        )
...
async def test_import_force_drops_then_reimports():
    ...
    with (
        patch(
            ...
            return_value="/fake/ncbitaxon.obo",
        ),
        patch(
            "knowledge_agent.kg.ontologies.ncbitaxon_writes._read_and_extract",
            return_value=[_term("NCBITaxon:1", "root")],
        ),
    ):
        assert (
            await ncbitaxon_writes.import_ncbitaxon(_client_with_driver(driver), force=True) is True
        )
...
async def test_import_aborts_on_zero_terms():
    ...
    with (
        ...
        patch("knowledge_agent.kg.ontologies.ncbitaxon_writes._read_and_extract", return_value=[]),
    ):
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert await ncbitaxon_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":NCBITaxonTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        await ncbitaxon_writes.delete_imported(_client_with_driver(driver))
