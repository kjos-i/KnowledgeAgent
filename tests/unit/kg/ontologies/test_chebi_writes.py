"""Tests for kg.ontologies.chebi_writes - the L7 ChEBI adapter.
...
from knowledge_agent.kg.ontologies import chebi_writes
...
def test_domain_tags_declared_chemistry_primary():
    ...
    relevance filter."""
    assert isinstance(chebi_writes.DOMAIN_TAGS, tuple)
    assert "chemistry" in chebi_writes.DOMAIN_TAGS
    assert "biochemistry" in chebi_writes.DOMAIN_TAGS
...
def test_lite_variant_url_pinned():
    ...
    assert "chebi_lite.obo" in chebi_writes.CHEBI_DOWNLOAD_URL
...
def test_cache_filename_uses_lite_variant():
    ...
    assert chebi_writes.CHEBI_CACHE_FILENAME == "chebi_lite.obo"
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await chebi_writes.is_imported(client) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert await chebi_writes.is_imported(client) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        await chebi_writes.is_imported(client)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await chebi_writes.write_terms(client, []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await chebi_writes.write_terms(client, terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await chebi_writes.write_terms(client, terms)
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await chebi_writes.write_terms(client, [_term("CHEBI:24431", "chemical entity")])
...
async def test_import_chebi_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mock_cache:
        result = await chebi_writes.import_chebi(client, force=False)
...
async def test_import_chebi_force_drops_then_reimports():
    ...
    with (
        ...
        patch(
            "knowledge_agent.kg.ontologies.chebi_writes._read_and_extract",
            return_value=fake_terms,
        ),
    ):
        result = await chebi_writes.import_chebi(client, force=True)
...
async def test_import_chebi_aborts_on_zero_terms():
    ...
    with (
        ...
        await chebi_writes.import_chebi(client, force=False)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await chebi_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":ChEBITerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        await chebi_writes.delete_imported(client)
