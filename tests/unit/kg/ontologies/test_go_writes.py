"""Tests for kg.ontologies.go_writes - the L7 GO adapter.
...
def test_domain_tags_declared():
    ...
    assert isinstance(go_writes.DOMAIN_TAGS, tuple)
    assert len(go_writes.DOMAIN_TAGS) > 0
    assert "biology" in go_writes.DOMAIN_TAGS
...
async def test_is_imported_true_when_query_returns_present():
    ...
    assert await go_writes.is_imported(client) is True
...
async def test_is_imported_false_when_query_returns_no_nodes():
    ...
    client = _client_with_driver(driver)
    assert await go_writes.is_imported(client) is False
...
async def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="connection lost"):
        await go_writes.is_imported(client)


async def test_is_imported_query_uses_goterm_label():
    driver = RecordingDriver(
        canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]]
    )
    client = _client_with_driver(driver)
...
async def test_write_terms_empty_input_returns_true_no_io():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await go_writes.write_terms(client, []) is None
...
async def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert await go_writes.write_terms(client, terms) is None
...
async def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    ...
    await go_writes.write_terms(client, terms)
...
async def test_write_terms_edge_query_uses_go_is_a_rel():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    terms = [
        _term("GO:0008150", "biological_process"),
    ...
    assert params["rows"] == [{"child": "GO:0007154", "parent": "GO:0008150"}]
...
async def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await go_writes.write_terms(client, [_term("GO:0008150", "biological_process")])


# ---- import_go ----


async def test_import_go_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontologies.writes.require_cached") as mock_cache:
        result = await go_writes.import_go(client, force=False)

    assert result is False  # no-op: typed-errors contract
    mock_cache.assert_not_called()


async def test_import_go_force_drops_then_reimports():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    ...
    with (
        ...
        patch(
            "knowledge_agent.kg.ontologies.go_writes._read_and_extract",
            return_value=fake_terms,
        ),
    ):
        result = await go_writes.import_go(client, force=True)
...
async def test_import_go_aborts_on_zero_terms():
    driver = RecordingDriver(
        canned_results_per_session=[[_RecordingResult(rows=[{"present": False}])]]
    )
    client = _client_with_driver(driver)
    with (
        ...
        await go_writes.import_go(client, force=False)


async def test_import_go_propagates_download_exception():
    driver = RecordingDriver(
        canned_results_per_session=[[_RecordingResult(rows=[{"present": False}])]]
    )
    client = _client_with_driver(driver)
...
async def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert await go_writes.delete_imported(client) is None
    assert len(driver.sessions[0].calls) == 1
    cypher, _ = driver.sessions[0].calls[0]
    assert ":GOTerm" in cypher
...
async def test_delete_imported_propagates_driver_exception():
    ...
    client = _client_with_driver(driver)
    with pytest.raises(RuntimeError, match="boom"):
        await go_writes.delete_imported(client)
