...
def test_client_is_so_imported_delegates_to_module():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    assert _client_with_driver(driver).is_so_imported() is True


def test_client_delete_so_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert client.delete_so() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":SOTerm" in cypher
    assert "DETACH DELETE" in cypher


def test_client_import_so_delegates_to_module():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    client = _client_with_driver(driver)
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert client.import_so(force=False) is False
    mc.assert_not_called()
