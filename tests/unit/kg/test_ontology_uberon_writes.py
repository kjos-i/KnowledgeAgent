...
def test_client_is_uberon_imported_delegates_to_module():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
    ...
    client = _client_with_driver(driver)
    assert client.is_uberon_imported() is True


def test_client_delete_uberon_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert client.delete_uberon() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":UBERONTerm" in cypher
    assert "DETACH DELETE" in cypher


def test_client_import_uberon_delegates_to_module():
    driver = RecordingDriver(
        canned_results_per_session=[
            [_RecordingResult(rows=[{"present": True}])]
        ]
    ...
    with patch(
        ...
        assert client.import_uberon(force=False) is False
    mock_cache.assert_not_called()
