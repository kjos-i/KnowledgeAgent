"""Unit tests for `kg/ontology_provenance` — the frozen dataclass
...
The data carrier itself is simple, but this file pins:
  ...
  - The re-export from `ontology_lifecycle.py` resolves to the same
    class (no accidental double-definition)
  - Every shipped per-ontology constant
    (`ontology_<name>_writes._<NAME>_PROVENANCE`) builds without
...
# ---------------------------------------------------------------------------
# Re-export from ontology_lifecycle
# ---------------------------------------------------------------------------


def test_ontology_lifecycle_re_export_is_same_class() -> None:
    """`OntologyProvenance` is re-exported from `kg.ontology_lifecycle`
...
#
# Each `kg/ontology_<name>_writes.py` module declares a
...
def test_every_per_ontology_provenance_constant_constructs() -> None:
    """Walk every `kg.ontology_<name>_writes` module, import it, and
    pull its `_<NAME>_PROVENANCE` constant. None should raise."""
    from knowledge_agent import kg

    found: dict[str, OntologyProvenance] = {}
    for mod_info in pkgutil.iter_modules(kg.__path__):
        name = mod_info.name
        if not name.startswith("ontology_") or not name.endswith("_writes"):
            continue
        mod = importlib.import_module(f"knowledge_agent.kg.{name}")
