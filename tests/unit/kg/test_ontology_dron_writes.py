...
def test_is_imported_true_when_query_returns_present():
    ...
    assert ontology_dron_writes.is_imported(_client_with_driver(driver)) is True
...
def test_is_imported_false_when_query_returns_no_nodes():
    ...
    assert ontology_dron_writes.is_imported(_client_with_driver(driver)) is False
...
def test_is_imported_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="conn lost"):
        ontology_dron_writes.is_imported(_client_with_driver(driver))


def test_is_imported_query_uses_term_label():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    client = _client_with_driver(driver)
    ontology_dron_writes.is_imported(client)
...
def test_write_terms_empty_input_returns_true_no_io():
    ...
    assert ontology_dron_writes.write_terms(_client_with_driver(driver), []) is None
...
def test_write_terms_one_round_trip_when_no_hierarchy():
    ...
    assert ontology_dron_writes.write_terms(_client_with_driver(driver), terms) is None
...
def test_write_terms_node_query_uses_multilabel_and_id_carries_prefix():
    driver = RecordingDriver()
    terms = [
        _term("DRON:00000005", "aspirin",
              synonyms=("acetylsalicylic acid", "asa")),
    ]
    ontology_dron_writes.write_terms(_client_with_driver(driver), terms)
...
def test_write_terms_propagates_driver_exception():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        ontology_dron_writes.write_terms(
...
def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert ontology_dron_writes.import_dron(_client_with_driver(driver), force=False) is False
...
def test_import_force_drops_then_reimports():
    driver = RecordingDriver()
    with (
        patch("knowledge_agent.kg.ontology_writes.ensure_cached", return_value="/fake/dron.owl"),
        patch("knowledge_agent.kg.ontology_dron_writes._read_and_extract",
              return_value=[_term("DRON:00000001", "drug")]),
    ):
        assert ontology_dron_writes.import_dron(_client_with_driver(driver), force=True) is True
...
def test_import_aborts_on_zero_terms():
    ...
    with (
        ...
        with pytest.raises(RuntimeError, match="extracted 0 terms"):
            ontology_dron_writes.import_dron(_client_with_driver(driver), force=False)
...
def test_delete_imported_runs_one_detach_delete_query():
    driver = RecordingDriver()
    assert ontology_dron_writes.delete_imported(_client_with_driver(driver)) is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":DRONTerm" in cypher
...
def test_delete_imported_propagates_driver_exception():
    driver = RecordingDriver(raise_on_run=RuntimeError("boom"))
    with pytest.raises(RuntimeError, match="boom"):
        ontology_dron_writes.delete_imported(_client_with_driver(driver))
...
def test_read_and_extract_uses_rdflib_not_pronto():
    ...
    with (
        ...
        patch("knowledge_agent.kg.ontology_dron_writes.extract_terms_owl",
...
def test_client_is_imported_delegates_to_module():
    driver = RecordingDriver(canned_results_per_session=[[_RecordingResult(rows=[{"present": True}])]])
    assert _client_with_driver(driver).is_dron_imported() is True


def test_client_delete_delegates_to_module():
    driver = RecordingDriver()
    client = _client_with_driver(driver)
    assert client.delete_dron() is None
    cypher, _ = driver.sessions[0].calls[0]
    assert ":DRONTerm" in cypher
    assert "DETACH DELETE" in cypher
...
def test_client_import_delegates_to_module():
    ...
    client = _client_with_driver(driver)
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert client.import_dron(force=False) is False
    mc.assert_not_called()
