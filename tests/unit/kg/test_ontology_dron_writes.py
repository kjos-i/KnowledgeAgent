...
async def test_import_short_circuits_when_already_imported():
    ...
    with patch("knowledge_agent.kg.ontology_writes.ensure_cached") as mc:
        assert await ontology_dron_writes.import_dron(_client_with_driver(driver), force=False) is False
...
async def test_import_force_drops_then_reimports():
    ...
    with (
        ...
        assert await ontology_dron_writes.import_dron(_client_with_driver(driver), force=True) is True
