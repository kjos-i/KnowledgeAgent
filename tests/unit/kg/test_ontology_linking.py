"""Tests for kg.ontology_linking - the shared L7 linking pass.
...
from knowledge_agent.kg import ontology_linking
...
def test_registry_has_known_ontologies():
    ...
    for name in expected:
        entry = ontology_linking.ONTOLOGY_REGISTRY[name]
...
def test_registry_mesh_points_at_meshterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["mesh"]["term_label"] == "MeSHTerm"


def test_registry_go_points_at_goterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["go"]["term_label"] == "GOTerm"


def test_registry_hpo_points_at_hpoterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["hpo"]["term_label"] == "HPOTerm"


def test_registry_uberon_points_at_uberonterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["uberon"]["term_label"] == "UBERONTerm"


def test_registry_mondo_points_at_mondoterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["mondo"]["term_label"] == "MONDOTerm"


def test_registry_chebi_points_at_chebiterm_label():
    assert ontology_linking.ONTOLOGY_REGISTRY["chebi"]["term_label"] == "ChEBITerm"
...
def test_match_exact_multi_hit_returns_all_term_ids():
    ...
    matches = ontology_linking._match_entity_key("stress", index, "exact")
    assert matches == [
        ("MESH:D013315", None),
        ("GO:0006950", None),
    ]
...
def test_match_fuzzy_tries_singular_strip_when_plural():
    ...
    matches = ontology_linking._match_entity_key("cancers", index, "fuzzy")
...
def test_match_fuzzy_exact_wins_before_variants():
    ...
    matches = ontology_linking._match_entity_key("cancer", index, "fuzzy")
...
def test_match_fuzzy_falls_through_all_variants_no_hits():
    ...
    matches = ontology_linking._match_entity_key("foo", index, "fuzzy")
    assert matches == []


def test_fuzzy_variants_for_plural_word():
    variants = dict(ontology_linking._fuzzy_variants("diseases"))
...
def test_fuzzy_variants_for_hyphenated_text():
    variants = dict(ontology_linking._fuzzy_variants("non-alcoholic"))
...
def test_fuzzy_variants_no_duplicates():
    ...
    variants = ontology_linking._fuzzy_variants("non-alcoholic-diseases")
...
async def test_ensure_ontology_imported_returns_already_imported_when_present():
    ...
    with (
        patch(
            "knowledge_agent.kg.ontology_linking.ontology_mesh_writes.is_imported",
            return_value=True,
        ),
        patch(
            "knowledge_agent.kg.ontology_linking.ontology_mesh_writes.import_mesh"
        ) as mock_import,
    ):
